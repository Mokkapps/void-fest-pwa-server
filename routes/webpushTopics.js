...
router.post('/:name/subscribe', (req, res) => {
  ...
  admin
    ...
    .then(response => {
      ...
      res
        .status(200)
        .send(`Successfully subscribed to topic "${topicName}": ${JSON.stringify(response)}`);
    })
    .catch(error => {
...
router.post('/:name/unsubscribe', (req, res) => {
  ...
  admin
    ...
    .then(response => {
      ...
      res
        .status(200)
        .send(`Successfully unsubscribed from topic "${topicName}": ${JSON.stringify(response)}`);
    })
    .catch(error => {
      console.error(`Error unsubscribing from topic "${topicName}":`, error);
      res
        .status(400)
        .send(`Error unsubscribing from topic "${topicName}": ${JSON.stringify(error)}`);
    });
});
